...
export default class ProductCategory extends React.Component {
  ...
  get() {
    ...
    axios.get(url + "GetProductCategoryByProjectId/1")
      .then(response => {
        this.setState({ ProductCategory: response.data.GetProductCategoryByProjectIdResult })
        //Alert.alert(JSON.stringify(response.data.GetProductCategoryByProjectIdResult))
      })
      .catch(err => {
  ...
  renderItem(item) {
    ...
    return (
      <TouchableOpacity onPress={() => {
        this.props.navigation.navigate('ProductSubcategory',{Id:item.Id});
    ...
    return (
      <ImageBackground source={require("../image/violetbg2.png")} resizeMode="stretch" style={{ width: "100%", height: "100%", justifyContent: "flex-start", alignItems: "center" }}>
        <ScrollView style={{marginTop: 10,}}>
            <FlatList data={this.state.ProductCategory}
