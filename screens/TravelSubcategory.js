...
export default class TravelSubcategory extends Component {
    state={
    ...
    _getData(){
        axios.get('http://203.113.14.18/DCPTCWcfService/Service1.svc/GetTravelByProjectIdAndTravelCategoryId/1/'
        +this.state.Id)
        .then(result=>{
            this.setState({data:result.data.GetTravelByProjectIdAndTravelCategoryIdResult})
        })
    }
    renderItem(item) {
        ...
        return (
          <TouchableOpacity onPress={() => {
            this.props.navigation.navigate('Traveldetail',{Id:item.Id});
            ...
            <View style={cardStyle}>
              ...
              <View style={{ flexDirection: "column", margin: 5 }}>
                <View style={titleSubtitleSytle}>
                  <Text style={{ color: "black", fontFamily: "Prompt-Regular",lineHeight:30,fontSize:15 }}>{item.Name}</Text>
