...
export default class ProductSubcategory extends Component {
    ...
    _getData(){
        axios.get('http://203.113.14.18/DCPTCWcfService/Service1.svc/GetProductByProjectIdAndProductCategoryId/1/'
        +this.state.Id)
        .then(result=>{
            this.setState({data:result.data.GetProductByProjectIdAndProductCategoryIdResult})
            //Alert.alert({data:result.data.GetProductByProjectIdAndProductCategoryIdResult})
            //alert(JSON.stringify(GetProductByProjectIdAndProductCategoryIdResult));
        })
    }
    renderItem(item) {
            ...
            <View style={cardStyle}>
              ...
              <View style={{ flexDirection: "column", margin: 5 }}>
                <View style={titleSubtitleSytle}>
                  <Text style={{ color: "#000000EE", fontFamily: "Prompt-Regular",fontSize:15,lineHeight:25,maxWidth: 225 }}>{item.Name}</Text>
